...
export function Convert(itemsss, pcPro) {
    ...
    itemSheet = itemSheet.map(param => {
        ...
        let dataparam = [param.nameDefault, Number(param.width), Number(param.hight), param.nameVariant.split(","), param.ProductType.split(",")];
    ...
    items = items.map(item => {
        if (item.phoneCase === undefined) return item
            ...
            let itemFilter = itemSheet.filter(item2 => {
                ...
                let itemFilter3 = item2[4].filter(item3 => {
                    if (item.product.split(" ").join("").toLowerCase().endsWith(item3.split(" ").join("").toLowerCase()) === true) {
                        return true
                    }
                });

                if ((itemFilter2.length !== 0) && (itemFilter3.length !== 0)) { return true }
                else { return false }
            });
